...
public class TrainRepository implements TrainDAO {
    ...
    public Optional<Train> find(Integer trainKwr) {
        return trainJpaRepository.findByTrainKwr(trainKwr).map(a -> trainMapper.mapFromEntity(a, new CycleAvoidingMappingContext()));
    }
    ...
    public List<Train> findAll() {
        ...
        return all.stream().map(a -> trainMapper.mapFromEntity(a, new CycleAvoidingMappingContext())).toList();
    }
